...
public sealed class ConversionSettings
{
    ...
    public string FontDir { 
        get => (GlobalFontSettings.FontResolver as FontResolver)?.Dir ?? string.Empty; 
        set => (GlobalFontSettings.FontResolver as FontResolver)?.Dir = value; }
    ...
    {
        Pipeline = BuildPipeline(math: true);
    }

    private MarkdownPipeline BuildPipeline(bool math = false)
    {
