...
internal class InlineConverter : IInlineConverter
{
        ...
        MigrDocInlineContainer OutputParagraph, FontStyle? formatting = null)
    {
        ...
        {
            Attributes.Markup = emphasisInline.DelimiterChar.ToString();
        }
    ...
    internal bool Convert()
    {
        ...
        if (Inline is HtmlEntityInline he)
        {
            AddTextSpan(he.Transcoded.ToString(), _formatting);
            return true;
        }

        if (Inline is LinkInline lnk)
        {
            if (lnk.IsImage)
            {
                AddInlineImage(lnk);
    ...
    private bool AddLiteral()
    {
        ...
        {
            AddTextSpan((Inline as LiteralInline)!.Content.ToString(), _formatting);
        }
        return true;
    }
    ...
    private void AppendPlainText(string text)
    {
        ...
        while (lo is FormattedText ft)
        {
            lo = ft.Elements.LastObject;
        }
        if (lo is Text txt)
        {
            txt.Content += text;
    ...
    private bool AddEmphasis()
    {
        ...
        {
            var c = new InlineConverter(emphInline, this, Parent, OutputParagraph, _formatting);
            c.Convert();
        }
    ...
    {
        if (Inline is EmphasisInline emph)
        {
            ElementDescriptor.Type = type;
            var style = GetStyle().Eval();
            _formatting = style.Font.MergeWith(_formatting);

            foreach (var item in emph)
            {
                var c = new InlineConverter(item, this, Parent, OutputParagraph, _formatting);
                c.Convert();
            }
        }
    ...
    private void AddInlineCode()
    {
        ...
        if (res?.Success ?? false)
        {
            ...
            {
                _formatting.Color = Color.FromArgb(255, s.Color.R, s.Color.G, s.Color.B);
                _formatting.Bold = s.Bold ? true : _formatting.Bold;
                _formatting.Italic = s.Italic ? true : _formatting.Italic;
                _formatting.Underline = s.Underline ? Underline.Single : _formatting.Underline;

                AddTextSpan(s.Text, _formatting);
            }
        }
        else
        {
            AddTextSpan(code?.Content ?? "", _formatting);
        }
    }
    ...
    private bool AddFootnoteLink()
    {
        ...
        hyper?.AddFormattedText(idx.ToString() ?? "", _formatting.MergeWithFont(font, Parent.FontSize, Parent.Width, alreadyScaled: false));
    ...
    private void AddInlineLink(LinkInline lnk, FontStyle formatting)
    {
        ...
        if (lnk.Url != null)
        {
            ...
            var toc = !string.IsNullOrEmpty(Attributes.Style) && Regex.Match(Attributes.Style!, "^(Toc[1-6]|Index)$").Success;
    ...
    private void AddInlineImage(LinkInline lnk)
    {
        ...
        var width = Attributes.ContainsKey("width") ? Attributes["width"] : null;
        var height = Attributes.ContainsKey("height") ? Attributes["height"] : null;
        ...
        try
        {
            if (height != null) img.Height = Dimension.Parse(height).Eval(Parent.FontSize, Parent.Width);
        }
        ...
        try
        {
            if (width != null) img.Width = Dimension.Parse(width).Eval(Parent.FontSize, Parent.Width);
        }
        catch (ArgumentException e)
        {
            Parent.Owner.OnWarningIssued(this, "Dimension", e.Message + $", line {lnk.Line}");
    ...
    {
        var f = formatting.MergeWithFont(OutputParagraph.Font ?? new Font(), Parent.FontSize, Parent.Width, alreadyScaled: true);
    ...
    public List<SingleElementDescriptor> GetDescriptors(List<SingleElementDescriptor>? descriptors = null)
    {
        ...
        else if (Parent is RootBlockConvertor rootBlockConvertor)
        {
            rootBlockConvertor.GetDescriptors(descriptors);
        }
        return descriptors;
    }
    ...
    internal static string GetInlinePlainText(Inline i)
    {
        ...
        if (i is LiteralInline literalInline)
        {
            res.Append(literalInline.Content);
        }
        if (i is CodeInline codeInline)
        {
            res.Append(codeInline.Content);
        }
        if (i is ContainerInline containerInline)
        {
            foreach (var ii in containerInline)
            {
    ...
    {
        return (inline is LinkInline link) && link.Url!.StartsWith("md:") && link.Url.Length > 3;
    }
