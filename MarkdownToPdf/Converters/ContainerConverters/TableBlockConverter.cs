...
{
    private bool _tableWidthSet;
    private List<Dimension> _attributeColumnWidths = [];

    internal Markdig.Extensions.Tables.Table CurrentBlock { get; }
    public MigraDoc.DocumentObjectModel.Tables.Table? OutputTable { get; private set; }

    internal TableBlockConverter(Markdig.Extensions.Tables.Table block, ContainerBlockConverter parent)
        : base(block, parent)
    {
        Attributes = new ElementAttributes(CustomGetTextBefore() ?? string.Empty);
        ...
        CurrentBlock = (Block as Markdig.Extensions.Tables.Table)!;
    }
    ...
    protected override void PrepareStyling()
    {
        ...
        {
            Width = EvaluatedStyle.Table.Width.Eval(FontSize, Parent?.Width??0).Point;
            _tableWidthSet = true;
        }
        ...
        if (Attributes.ContainsKey("width"))
        {
            try
            {
                ...
                Width = w.Eval(FontSize, Parent?.Width??0).Point;
                _tableWidthSet = true;
            }
    ...
    private void PrepareColumnStyling()
    {
        if (Attributes.ContainsKey("columns"))
        {
            try
            {
                ...
                _attributeColumnWidths = colWidths;
            }
            catch (ArgumentException e)
            {
    ...
    {
        if (!margin.IsEmpty && OutputTable != null)
        {
    ...
    private void PrepareColumns()
    {
        ...
        var scale = totalColumnsWidth > Width || _tableWidthSet ? Width / totalColumnsWidth : 1.0;
    ...
    private List<Dimension> GetColumnWidths()
    {
        ...
        {
            if (_attributeColumnWidths != null)
            {
                var d = _attributeColumnWidths.Any() ? _attributeColumnWidths[Math.Min(_attributeColumnWidths.Count - 1, i)] : Dimension.FromContainerWidth(100);
                res.Add(d);
            }
            else
            {
