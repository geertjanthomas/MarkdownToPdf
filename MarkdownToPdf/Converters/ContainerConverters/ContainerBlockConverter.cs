...
using VectorAi.MarkdownToPdf.Converters.LeafConverters;
...
internal abstract class ContainerBlockConverter : BlockConverterBase
{
    protected ContainerBlockConverter(ContainerBlock block, ContainerBlockConverter? parent)
        : base(block, parent)
    {
        Attributes = new ElementAttributes(GetTextBefore() ?? string.Empty);
    }
    ...
    protected virtual bool ConvertBlock(Block block)
    {
        ...
        {
            var conv = new HeadingBlockConverter((block as HeadingBlock)!, this);
            conv.Convert();
            return true;
        }
        ...
        if (type == typeof(Markdig.Extensions.Tables.Table))
        {
            var conv = new TableBlockConverter((block as Markdig.Extensions.Tables.Table)!, this);
            conv.Convert();
            return true;
        }
    ...
    protected string? GetTextBefore()
    {
        ...
        else
        {
            ...
            {
                start = Lines?.Take(Block.Parent.Line).Sum(x => x.Length) ?? 0;
            }
        }
        ...
        {
            end = Lines?.Take(Block.Line + 1).Sum(x => x.Length) ?? 0;
        }
