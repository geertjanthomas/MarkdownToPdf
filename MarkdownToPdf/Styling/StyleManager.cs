...
public class StyleManager
{
    ...
    private (CascadingStyle Style, Action<CascadingStyle, StylingDescriptor> ModificationMethod) GetBestMatchingSyle(Dictionary<List<StyleSelector>, (CascadingStyle Style, Action<CascadingStyle, StylingDescriptor> ModificationMethod)> bindings, int level = 0)
    {
        ...
        // with Name And Type
        var res = bindings.Where(x => x.Key.Count > level && !string.IsNullOrEmpty(x.Key[level].StyleName) && x.Key[level].ElementType != ElementType.Any);
        ...
        if (!res.Any())
        {
            res = bindings.Where(x => x.Key.Count > level && !string.IsNullOrEmpty(x.Key[level].StyleName));
            ...
            case StyleSelector.SelectorTypes.Base:
                {
                    ...
                    if (!string.IsNullOrEmpty(bidningSelector.StyleName) && bidningSelector.StyleName != descriptors.First().Attributes.Style)
                    {
                        return false;
                    }
                    descriptorPositions[binding.Key]++;
            ...
            case StyleSelector.SelectorTypes.Parent:
                {
                    ...
                    if (!string.IsNullOrEmpty(bidningSelector.StyleName) && bidningSelector.StyleName != current.Attributes.Style)
                    {
                        return false;
                    }
                    descriptorPositions[binding.Key]++;
            ...
            case StyleSelector.SelectorTypes.Ancestor:
                {
                    ...
                    var anc = descriptors
                      ...
                      .SkipWhile(xx => xx.Type != ElementType.Any && xx.Type != bidningSelector.ElementType
                        || (!string.IsNullOrEmpty(bidningSelector.StyleName) && bidningSelector.StyleName != xx.Attributes.Style));
