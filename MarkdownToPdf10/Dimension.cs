...
{
    private readonly List<(DimensionUnit Unit, double Value)> _content;

    public bool IsEmpty { get => !_content.Any(); }

    public Dimension()
    {
        _content = new List<(DimensionUnit Unit, double Value)>();
    }

    internal Dimension(DimensionUnit unit, double value) : this()
    {
        _content.Add((unit, value));
    }
    ...
    public static Dimension operator +(Dimension a, Dimension b)
    {
        ...
        return new Dimension(a._content.Concat(b._content).ToList());
    }
    ...
    public static Dimension operator -(Dimension a, Dimension b)
    {
        ...
        res = res.Concat(a._content).ToList();
        foreach (var i in b._content)
        {
